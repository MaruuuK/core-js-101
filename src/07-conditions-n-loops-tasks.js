...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').reduce((sum, val) => sum + Number(val), 0));
}
...
function getCommonDirectoryPath(pathes) {
  const firstFileDirs = pathes[0].split('/');
  let commonPath = '';

  for (let i = 0; i < firstFileDirs.length; i += 1) {
    const currentDir = firstFileDirs[i];

    const isCommonDir = pathes.every((filename) => {
      const dirs = filename.split('/');
      return dirs[i] === currentDir;
    });
